(function ($) {
	 ...
	 * @type {String}
	 */
	lm.controls.Tab._template = '<li class="lm_tab dx-tab"><i class="lm_left"></i>' +
	...
	lm.utils.copy(lm.items.Stack.prototype, {
		...
		setActiveContentItem: function (contentItem) {
			...
			//-------------------------------------------------------------------------------------------
			const idn = contentItem.element.find('.gridContainer').attr('id');
			const url = app.parseUrl(idn);
			const obj = $("#" + idn).data('mylsObject');

			if (!obj && this.layoutManager.config.mylsCanCreate) {
				const object = app.getObject(url.table, this.layoutManager.config.mylsExtId, 'layout', url.type, this.layoutManager.config.mylsMode, this.layoutManager.config.mylsTHistory);
				object.init();
			} //else
			//obj.repaint();
