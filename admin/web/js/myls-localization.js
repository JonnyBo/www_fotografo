class MylsLocalization {
	...
	async getTranslate() {
		...
		if (translate != '') {
			this.translate = translate;
			this.translateOld = app.cloneObject(this.translate);
		}
	...
	saveFileTranslate() {
		if (this.mode == 'development') {
			const self = this;
			let array = {};
			// let difference = this.translate.filter(x => !this.translateOld.includes(x));
			$.each(this.translate, (lang, items) => {
				$.each(items, (index, item) => {
					if (!self.translateOld[lang] || !self.translateOld[lang][index]) {
						if (!array[lang]) array[lang] = {};
						array[lang][index] = item;
					}
				});
			});
			if (Object.keys(array).length)
				app.processData("site/savetranslate", 'post', {data: array});
			this.translateOld = app.cloneObject(this.translate);
		}
	}
