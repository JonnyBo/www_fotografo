class Grid extends MylsEditableObject {
	...
	getOptions() {
		...
		return {
			...
			columns: this.columns.getColumnsForGrid(),
			summary: {
			...
			searchPanel: {visible: app.appInfo.device.deviceType == 'phone'? false: true},
			...
			stateStoring: {
				...
				savingTimeout: 500,
				customLoad: function () {
					const data = app.appInfo.setting[self.table];
						...
						$.each(data.columns, function (index, item) {
							if (item) {
								...
								if (!item['dataField']) {
									...
									columns.push(item);
								} else if (self.usedColumns[item.dataField] && self.usedColumns[item.dataField].visible) {
									columns.push(item);
								}
							}
						});
	...
	focusedRowChanged(e) {
		this.setReadToRow(e);
		if (e.row && e.row.rowType == 'data' && e.row.data.hasOwnProperty('id')) {
			this.showId(e.row.data.id);
		}
	}
	...
	customSave(state) {
		...
		if (this.curState != cState) {
			...
			app.appInfo.setting[this.table] = state;
			window.localStorage.setItem('appInfo', JSON.stringify(app.appInfo));
	...
	selectionChanged(e) {
		...
		this.showSelected(countSelected);
	}
	...
	gridContentReady(e) {
		...
		if (this.template && this.template.hasOwnProperty('report')) {
			//console.log(this.object.getCombinedFilter());
			//console.log(this.object.filter(this.object.getCombinedFilter()));
			//this.object.filter(this.object.getCombinedFilter());
			//console.log(dataSource.filter(this.object.getCombinedFilter()));
			this.chartView.repaint(this.tableData);
		}
	}
	...
	async editInline(params) {
		...
		return new Promise(async (resolve) => {
			...
			if (self.mode == 'ins') {
				await self.prepareInsert(params, false);
			}
