class Columns {
	...
	getColumnTypeItem(type, item) {
		let params = null;
		if (item.columnType != null && $.isArray(item.columnType)) {
			$.each(item.columnType, function (i, el) {
				if ($.type(el) == 'object') {
					if (type in el) {
						params = el[type];
	...
	setCellTemplates(item) {
			...
			item.headerFilter = {
				dataSource: function (data) {
					data.dataSource.postProcess = function (results) {
						let newResults = [];
						$.each(results, function (index, filterItem) {
							let values = filterItem.text.split(',');
		...
		function templateBlock() {
			if (item.usedColumns.length > 0) {
				item.headerFilter = {
					dataSource: function (data) {
						data.dataSource.postProcess = function (results) {
							let newResults = [];
							$.each(self.mylsObject.dataSource.items(), function (index, filterItem) {
								$.each(item.usedColumns, function (index, value) {
									if (filterItem[value] != null) {
										let val = filterItem[value];
										let exists = false;
										$.each(newResults, function (index, r) {
											if (r.text == val) {
												exists = true;
							...
							let resultArray = numArr.concat(strArr);
							return resultArray;
						};
					}
				};
				//console.log(item);
				//полчить значения всех нужных столбцов и добавить их к фильтру
	...
	setSummaryFields() {
		...
		if (this.summary && this.summary.length > 0) {
			$.each(this.columns, function (index, item) {
				if (item.footerField) {
					const sumItem = getSummaryItem(item.footerField);
					if (sumItem)
		...
		function getSummaryItem(dataField) {
			let sumItem = null;
			$.each(self.summary, (index, item) => {
	...
	getTableDataByColumns(data) {
		...
		if (data) {
			$.each(this.columns, function (index, item) {
				if (data.hasOwnProperty(index) && !item.customSave)
	...
	getColumnsForGrid() {
		let result = {};
		let columns = this.getUsedColumns();
		let keysSorted = Object.keys(columns).sort((a, b) => columns[a].parentId - columns[b].parentId);
		for (let i of keysSorted) {
			if (columns[i].parentId && columns[i].hasOwnProperty('columnId')) {
				let index = Object.keys(columns).find(key => columns[key].columnId === columns[i].parentId);
				if (index !== undefined) {
					if (!columns[index].hasOwnProperty('columns')) {
						columns[index].columns = [];
					}
					columns[index].columns.push(columns[i]);
				}
			} else {
				result[i] = columns[i];
			}
		}
		result.__lastColumn__ = {
			width: "auto",
