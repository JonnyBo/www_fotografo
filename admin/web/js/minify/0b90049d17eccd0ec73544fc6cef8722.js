...
if(result.error&&result.error!==''){reject(result);}else{resolve(result);}
else{resolve();}}else{resolve();}},error:function(object,type,error){console.log(error);reject(error);}});});}catch(e){return Promise.reject(e.message);}}
...
async init(){const self=this;await this.initData();this.createDrawer();this.topTabs.init();this.toolbar.init();this.initQuickActions();this.initDialog();this.createTooltip();if(window.location.hash!==''){this.currentHash=window.location.hash;}
try{await this.openOldTabs();await this.openOldPopups();}catch(e){console.log(e);}
...
let lastDate=await this.processData('site/getstructurelastdate','POST',null);await Promise.all([this.getSettings(),this.translate.getTranslate()]);this.openLoadPanel("main");await Promise.all([this.getAllSettings(),this.getAllTablesInfo(),this.getAllColumns(),this.getAllContextMenu()]);await Promise.all([this.getAllTemplates(),this.getQuickActions(),this.getMenu()]);this.closeLoadPanel('main');this.translate.saveFileTranslate();window.localStorage.setItem('appInfo',JSON.stringify(this.appInfo));window.localStorage.setItem('appInfo.date',new Date());}
initDialog(){this.dialog=new Dialog();}
initQuickActions(){if(this.appInfo.quickActions&&this.appInfo.quickActions.length){$.each(this.appInfo.quickActions,function(index,item){$(".app-container").append("<div id='action-"+index+"'></div>");$("#action-"+index).dxSpeedDialAction({label:app.translate.saveString(item.text),icon:item.icon,index:item.index,onClick:function(){}}).dxSpeedDialAction("instance");});}}
createDrawer(){this.drawer=$("#drawer").dxDrawer({opened:($(window).width()<'1024')?false:true,position:'before',closeOnOutsideClick:($(window).width()<'1024')?true:false,openedStateMode:($(window).width()<='1024'?'overlap':'shrink'),template:()=>{this.menu.init();return this.menu.object.element();}}).dxDrawer("instance");}
setDrawerOptions(){this.drawer.option("openedStateMode",($(window).width()<='1024'?'overlap':'shrink'));}
...
async openOldTabs(){const self=this;return new Promise((resolve)=>{if(self.config.tabs!==null&&self.config.tabs!==undefined){if(self.config.tabs.length>0){let oldSel=self.config.selTabs;$.each(self.config.tabs,function(index,item){let tHistory=JSON.parse(item.tHistory);const table=self.parseUrl(item.idn).table;if(self.appInfo.tables[table]){self.openTabFromUrl(item.idn,tHistory);}});if(oldSel!==undefined&&self.topTabs.panelContent.length>oldSel){self.topTabs.object.option("selectedIndex",oldSel);self.updateUrl(self.config.tabs[oldSel].idn);}}}
resolve();});}
...
async init(){this.allSearchLookupDataSource=this.getAllSearchLookupDataSource();this.toolbarItems=[];await this.setMainToolbarItems();this.getAllIcons(app.appInfo.menu);this.createObject();}
createObject(){this.object=$("#toolbar").dxToolbar({items:this.toolbarItems}).dxToolbar('instance');}
async setMainToolbarItems(){this.toolbarItems.push({widget:"dxButton",location:"before",cssClass:"menu-button",options:{icon:"menu",onClick:function(){app.drawer.toggle();}}});this.getLogo();await this.getAllSearchLookup();this.getToolbarMenu();}
getLogo(){let logoItem={location:"before",cssClass:"mylsTitle",cssStyle:"mylsTitle",};if(logo.app=='img'){logoItem.html='<div><img src="'+logo.logoImg+'"></div>';}else
...
createFileUploader(){if(this.tableInfo.docProc&&this.tableInfo.a==1){const pbId=this.idn+'_progressbar';$('#'+this.idn).prepend('<div id="'+this.idn+'_file-uploader"></div>');$("#"+this.idn+"_file-uploader").after('<div id="'+pbId+'"/>');$("#"+this.idn+"_file-uploader").dxFileUploader(this.getDocumentOptions()).dxFileUploader('instance');}}};;class Toolbar{constructor(object){this.currentFilter=[];this.table=object.table;this.ext_id=object.ext_id;this.view=object.view;this.type=object.type;this.mode=object.mode;this.tHistory=object.tHistory;this.items=[];this.mylsObject=object;}
...
createFilterButton(){if(this.type!=='chart'){this.items.push({widget:"dxButton",name:'buttonSearch',locateInMenu:'never',options:this.getFilterBtnOptions(),location:"before",});}}
...
editMode(){const self=this;const obj=this.mylsObject.object;if(!obj||!Object.keys(obj).length)return;if(this.mylsObject.type==='grid'||this.mylsObject.type==='tree'){let isEdit=obj.option('editing').mode=='batch';if(isEdit){this.createExitButton();}
...
console.log(this.type);this.isEdit=false;this.mylsObject.mode='sel';this.showEditButtons('row');if(this.type=='cards'){this.exitBtn.option('visible',false);this.GridSortBtn.option('icon','sorted');$('#'+this.mylsObject.idn+' .dx-scrollview-content.card-view').sortable('destroy');this.mylsObject.refresh();}}
getExitBtnOptions(){const self=this;return{icon:"img/exit.svg",elementAttr:{toolbarrole:"always",buttonrole:"exit",},visible:(this.positionColumn&&this.mylsObject.tableInfo.e==1)?false:true,onInitialized:e=>{self.exitBtn=e.component;},onClick:function(e){e.event.stopPropagation();self.exitFromEditing(e);}};}
...
getRefreshBtnOptions(){const self=this;return{elementAttr:{toolbarrole:"always",buttonrole:"refresh",},icon:"/img/refresh.svg",visible:!self.isEdit,onClick:async function(e){self.mylsObject.refresh();self.updateFilterList();}};}
...
getGridSortBtnOptions(disable=true){const self=this;return{elementAttr:{toolbarrole:"focused",buttonrole:"gridsort",class:'',},icon:"sorted",disabled:disable,onInitialized:e=>{self.GridSortBtn=e.component;},onClick:function(e){let cls=e.component.option('icon');if(cls=='sorted'){self.startOrdered(e);self.exitBtn.option('visible',true);}else if(cls=='save'){self.saveOrdered(e);self.exitBtn.option('visible',false);}}};}
startOrdered(e){const self=this;e.component.option('icon','save');if(self.type=='grid'){self.mylsObject.object.clearSorting();self.mylsObject.object.columnOption(this.positionColumn.dataField,"sortOrder",'asc');}
...
await app.processData('frame/updateposition','POST',positionGrid);await this.mylsObject.refresh();e.component.option('icon','sorted');if(self.type=='grid')
...
show(){}
hide(){}
remove(){this.object.dispose();$('#'+this.pbId).remove();this.object=null;}
...
customSave(state){app.prepareStorage(state);const cState=JSON.stringify(state);if(this.curState!=cState){this.sendStorageRequest("storage","json","POST",state,this.table);app.appInfo.setting[this.table]=state;this.curState=cState;}}
...
processData(data){const self=this;data=this.removeNull(data);data.sort((prev,next)=>{return(prev[self.sortField]<next[self.sortField])?-1:1;});let out=[];let old='';let k=0;for(let key in data){if(old!=data[key][this.argument]){if(key>0)
...
createCheckBox(){const self=this;return $('<div class="myls-card-checkbox">').dxCheckBox({value:false,onValueChanged:function(e){self.selectCard(e.element,true);}});}
selectCard(card,select){const $card=$(card);const $parent=$card.closest('.dx-card');const $checkbox=$parent.find('.myls-card-checkbox').dxCheckBox('instance');const checkboxValue=$checkbox.option('value',true);const id=$parent.attr('data-id');try{const itemIndex=this.findInDataSource(id,this.dataDS.items(),'id');if(itemIndex!==-1){const item=this.dataDS.items()[itemIndex];$checkbox.option('value',!checkboxValue);$parent.toggleClass('card-selected');item.selected=!item.selected?true:false;item['__from_group']=item.group_id;if(select){item.selected=select;$parent.removeClass('card-selected');$parent.addClass('card-selected');}
...
return selParams;}
createFileTemplate(itemData,itemElement,tabId,item,fileType){const self=this;const idn=tabId+'_'+item.dataField+'_file';const column=self.columns.columns[itemData.dataField];column.element=$(itemElement).closest('.dx-item');column.elementIdn=idn;this.loadPromise.then(()=>{column.objectType=fileType;itemElement.append(`<div id="${idn}"  class="myls-form-file-container" myls-field="${item.dataField}"><div id="${tabId}_${item.dataField}_fileuploader"></div><div id="selected-files"></div></div>`);column.editor=$("#"+idn+'uploader').dxFileUploader({multiple:false,accept:fileType==='image'?'image/*':'*',uploadMode:"instantly",uploadUrl:"form/uploadimage?field="+idn,name:idn,showFileList:false,onUploaded:fileUploaded,}).dxFileUploader("instance");$("#"+tabId+'_'+item.dataField+'_fileuploader .dx-fileuploader-button').addClass("order-1 mt-2 d-block");$("#"+tabId+'_'+item.dataField+'_fileuploader .dx-fileuploader-input-wrapper').addClass("d-flex flex-column p-0 mt-2 border-0");$("#"+tabId+'_'+item.dataField+'_fileuploader .dx-fileuploader-input-container').addClass('d-flex justify-content-center');$("#"+tabId+'_'+item.dataField+'_fileuploader .dx-fileuploader-input').addClass('d-none');$("#"+tabId+'_'+item.dataField+'_fileuploader .dx-fileuploader-input-label').addClass("d-none");self.refreshFileTemplate(column);});function fileUploaded(e){const filename=$.parseJSON(e.request.response);if(filename!==''){self.tableData[$('#'+idn).attr('myls-field')]=filename;if(column.extField){if(app.hasValue(self.columns.columns[column.extField]))
self.setFieldValue(null,self.columns.columns[column.extField],e.file.name);}
self.refreshFileTemplate(column);}}}
getFileTemplate(column){const filename=this.tableData[column.dataField];const idn=column.elementIdn;if(filename){if(column.objectType==='image'){return`<img class="myls-field-image"  src="files/${filename}" /><div id="${idn}-buttonDeleteFile"></div>`;}
...
createFileDeleteButton(column){const self=this;if(this.tableData[column.dataField])
...
initLookup(item,column,form){const self=this;item.editorOptions.dataSource=this.initLookupDataSource(column,form,this.deps);item.editorOptions.showPopupTitle=false;if(item.grouped){item.editorOptions.grouped=true;item.editorOptions.dataSource.group="category";item.editorOptions.dataSource.paginate=false;}
...
if(e.text){const isFind=this.findInDataSource(e.text,e.component.option('items'));if(isFind==-1)
...
currText=undefined;let isFind=(e.value)?e.value:-1;if(customAddBtn){customAddBtn.option("visible",currText&&(isFind===-1)&&column.canAdd);customAddBtn.mylsCurrValue=currText;}
if(customMoreBtn)
...
async initHtml(idnf,column,form){column.customEditor=true;column.customSave=true;try{if(column.editor)column.editor.destroy();column.editor=await ClassicEditor.create(document.querySelector('#'+idnf),this.getHtmlOptions(column));this.setCustomEditorsValue(column,this.tableData[column.dataField]);}catch(error){console.log(error);}}
...
this.buttons.push(this.getButtonCancel(idx));this.buttons.push(this.getButtonCollapse(idx));this.buttons.push(this.getButtonFullscreen(idx));this.buttons.push(this.getButtonClose(idx));}else{$.each(this.btns,function(index,item){if(item=='info')
...
self.destroy();},toolbarItems:this.buttons,showTitle:true,title:app.translate.saveString("Information"),visible:false,dragEnabled:true,closeOnOutsideClick:false,resizeEnabled:true,maxSize:"90%",maxHeight:"100%",showCloseButton:false,shading:this.modal,};}
...
updateModalTitle(){this.object.option('title',this.mylsObject.tableInfo.name);}
...
app.tooltip.option('contentTemplate','<a href="'+url+'" '+target+'><i class="dx-icon-'+type+'"></i> '+info+'</a>');app.tooltip.show('#'+id);});$(document.body).on('click','.myls-open-object',function(e){e.stopPropagation();app.openTabFromUrl($(e.target).attr('data-url'));});$(window).on('resize',()=>{app.setDrawerOptions();});;