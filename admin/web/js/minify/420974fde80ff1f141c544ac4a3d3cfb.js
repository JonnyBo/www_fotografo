...
processData(url,method,data){if(data===null||data===undefined)data='';try{return new Promise((resolve,reject)=>{$.ajax({url:url,method:method,data:(data),success:function(result){if(result){result=$.parseJSON(result);if(result.error&&result.error!=''){reject(result);}else{resolve(result);}}else{resolve();}},error:function(object,type,error){console.log(error);reject(error);}});});}catch(e){return Promise.reject(e.message);}}
...
hasValue(object){return object!=''&&object!==null&&object!==undefined;}};;class Auth{constructor(page){this.page=page;this.userData={};this.token='';this.formIdn='';switch(this.page){case'login':this.formIdn='mylsAuthForm';break;case'registration':this.formIdn='mylsRegistrationForm';break;case'restore':this.formIdn='mylsRestoreForm';this.token=app.getUrlParameter('token');if(this.token){$('#form-container').attr('action','restore?token='+this.token);}
break;}}
async getUserData(token){this.userData=await app.processData('/site/regurl-decode','POST',{token:token});}
