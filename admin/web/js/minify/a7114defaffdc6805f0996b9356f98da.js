...
ext_id=undefined;switch(this.getRealObjectType(type)){case'dashboard':object=new Dashboard(table,ext_id,view,mode,tHistory,viewMode,params);break;case'form':object=new Form(table,ext_id,view,mode,tHistory,viewMode,params);break;case'grid':object=new Grid(table,ext_id,view,mode,tHistory,viewMode,params);break;case'tree':object=new Tree(table,ext_id,view,mode,tHistory,viewMode,params);break;case'cards':object=new CodeEditorTest(table,ext_id,view,mode,tHistory,viewMode,params);break;case'scheduler':object=new Scheduler(table,ext_id,view,mode,tHistory,viewMode,params);break;case'chart':object=new Charts(table,ext_id,view,mode,tHistory,viewMode,params);break;case'pivot':object=new Pivot(table,ext_id,view,mode,tHistory,viewMode,params);break;case'kanban':object=new Kanban(table,ext_id,view,mode,tHistory,viewMode,params);break;case'draglist':object=new DragList(table,ext_id,view,mode,tHistory,viewMode,params);break;case'twowaygrid':object=new TwoWayGrid(table,ext_id,view,mode,tHistory,viewMode,params);break;case'htmleditor':object=new HtmlEditor(table,ext_id,view,mode,tHistory,viewMode,params);break;case'codeeditor':object=new CodeEditor(table,ext_id,view,mode,tHistory,viewMode,params);break;case'layout':object=new Layout(table,ext_id,view,mode,tHistory,viewMode,params);break;}
...
async processFormItemsTemplates(form,tabId,formObj,tabIndex){const self=this;$.each(form,function(key,item){console.log(item);if(item.hasOwnProperty("label")&&item.label.hasOwnProperty("text")&&item.label.text=='')
...
async initCodeEditor(idnf,column,form){column.customEditor=true;column.customSave=true;if(column.editor)column.editor.destroy();if(column.columnType&&$.isArray(column.columnType)){let test=this.columns.getColumnTypeItem('test',column);let lang=this.columns.getColumnTypeItem('lang',column);let objTest=this.columns.getColumnTypeParameters(test);let objLang=this.columns.getColumnTypeParameters(lang);$('#'+idnf).css('min-height','300px');column.editor=await ace.edit(idnf);column.editor.setTheme("ace/theme/chrome");column.editor.session.setMode("ace/mode/"+objLang[0]);this.setCustomEditorsValue(column,this.tableData[column.dataField]);const options=this.getTestButtonOptions(idnf,column.editor,objTest.url);this.addButtonTest(idnf,options);}}
...
createTestInfo(idnf){$('#'+idnf).after('<div id="'+idnf+'-info"></div>');}
getTestButtonOptions(idnf,editor,url){const self=this;return{stylingMode:"outlined",text:"Тестировать",type:"default",width:120,onClick:async function(){const result=await app.processData(url,'POST',{code:editor.getValue()});if($('#'+idnf+'-info').length==0)
...
createObject(){$('#'+this.idn).append('<div id="'+this.editorIdn+'-toolbar"></div><div id="'+this.editorIdn+'-editor" class="gridContainer"></div>');this.object=DecoupledEditor.create(document.querySelector('#'+this.editorIdn+'-editor')).then(editor=>{const toolbarContainer=document.querySelector('#'+this.editorIdn+'-toolbar');toolbarContainer.appendChild(editor.ui.view.toolbar.element);}).catch(error=>{console.error(error);});}};;class CodeEditor extends MylsObject{constructor(table,ext_id,view,mode,tHistory,viewMode,params){super(table,ext_id,view,mode,tHistory,viewMode,params);this.type='codeeditor';}
async init(){super.init();this.editorIdn=app.getIdn('codeeditor',this.table,this.ext_id,this.view);this.createObject();}
createObject(){console.log(this.idn);$('#'+this.idn).append('<div id="'+this.editorIdn+'-toolbar"></div><div id="'+this.editorIdn+'-editor" class="gridContainer"></div>');this.object=ace.edit(this.editorIdn+"-editor");this.object.setTheme("ace/theme/monokai");this.object.session.setMode("ace/mode/php");}};;class CodeEditorTest extends CodeEditor{constructor(table,ext_id,view,mode,tHistory,viewMode,params){super(table,ext_id,view,mode,tHistory,viewMode,params);this.type='codeeditor';}
async init(){super.init();this.createTestButton();}
createTestButton(){$('#'+this.editorIdn+'-toolbar').append('<div id="mylsTestButton"></div>');$('#mylsTestButton').dxButton(this.getTestButtonOptions());}
createInfo(){$('#'+this.idn).append('<div id="'+this.editorIdn+'-info"></div>');}
getTestButtonOptions(){const self=this;return{stylingMode:"contained",text:"Тестировать",type:"normal",width:120,onClick:async function(){const result=await app.processData('site/test-code','POST',{code:self.object.getValue()});if($('#'+self.editorIdn+'-info').length==0)
self.createInfo();if(result.success)
$('#'+self.editorIdn+'-info').addClass('success').text(result.success);if(result.error)
$('#'+self.editorIdn+'-info').addClass('error').text(result.error);}}}};;class ContextMenu{constructor(object){this.mylsObject=object;}
