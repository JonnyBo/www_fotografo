...
await Promise.all([this.getSettings(),this.translate.getTranslate()]);this.openLoadPanel("main");await Promise.all([this.getAllSettings(),this.getAllTablesInfo(),this.getAllColumns(),this.getAllContextMenu()]);await Promise.all([this.getAllTemplates(),this.getQuickActions(),this.getMenu()]);this.closeLoadPanel('main');this.translate.saveFileTranslate();window.localStorage.setItem('appInfo',JSON.stringify(this.appInfo));window.localStorage.setItem('appInfo.date',new Date());}
