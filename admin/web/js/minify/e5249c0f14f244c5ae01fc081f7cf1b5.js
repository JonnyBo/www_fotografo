...
cloneObject(obj){return obj?JSON.parse(JSON.stringify(obj)):undefined;}};;class Auth{constructor(page){this.page=page;this.userData={};this.token='';this.formIdn='';switch(this.page){case'login':this.formIdn='mylsAuthForm';break;case'registration':this.formIdn='mylsRegistrationForm';break;case'regbyuser':this.formIdn='mylsRegByUserForm';break;case'restore':this.formIdn='mylsRestoreForm';this.token=app.getUrlParameter('token');if(this.token){$('#form-container').attr('action','restore?token='+this.token);}
break;}}
async getUserData(token){if(this.page=='registration')
this.userData=await app.processData('/site/regurl-decode','POST',{token:token});if(this.page=='regbyuser')
this.userData=await app.processData('/site/regbyurl-decode','POST',{token:token});}
checkUserMail(email){return app.processData('site/checkuser','POST',{email:email});}
createForm(){let userName=$.cookie('userName');userName=(userName!=null)?userName:'';this.formData={};switch(this.page){case'login':this.formData={"username":userName,"password":"","rememberMe":false};break;case'registration':this.formData={"name":this.userData.name,"surname":this.userData.surname,"email":this.userData.email,"phone":"","password":"","token":this.token,"company_id":this.userData.company_id,};$('#company_id').val(this.userData.company_id);break;case'regbyuser':this.formData={"password":"","token":this.token,"company_id":this.userData.company_id,};$('#company_id').val(this.userData.company_id);break;case'restore':this.formData={"email":userName,"password":"","re-password":""};break;}
this.createObject();}
...
let funcPromice=[app.translate.getTranslate(),app.getSettings()];if(this.page=='registration'||this.page=='regbyuser'){let token=app.getUrlParameter('token');if(!token){window.location.href='login';}
funcPromice.push(this.getUserData(token));}
const self=this;Promise.all(funcPromice).then(()=>{self.createForm();app.translate.saveFileTranslate();});}
...
if(this.page=='regbyuser'){this.formItems.push({itemType:"group",items:[this.getName(),this.getSurname(),this.getPassword(),this.getRePassword()]});btnText="Зарегистрироваться";}
...
this.formItems.push({itemType:"group",items:this.getLangItems()});this.formItems.push({itemType:"button",horizontalAlignment:"center",buttonOptions:{text:app.translate.saveString(btnText),type:"default",stylingMode:'outlined',useSubmitBehavior:true}});return this.formItems;}
...
getLanguages(){return{cssClass:'myls-login-language-block',template:function(e){let loginLang=app.config.lang;let result='';if(languages.length>1){$.each(languages,function(index,item){let addclass='';if(item.code==loginLang)
addclass=' selected';else
addclass='';result+='<a class="myls-login-language '+addclass+'" href="javascript:void(0)" onclick="app.translate.changeLocaleForm(\''+item.code+'\')">'+item.code+'</a>';});}
return result;}}}
