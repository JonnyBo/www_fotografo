...
const loadSettings=this.getSettings();if(await this.shouldLoadStructure()){await Promise.all([this.getIfAllsearchLookupExists(),loadSettings,this.translate.getTranslate()]);this.openLoadPanel("main");await Promise.all([this.getAllSettings(),this.getAllTablesInfo(),this.getAllColumns(),this.getAllContextMenu()]);await Promise.all([this.getAllTemplates(),this.getQuickActions(),this.getMenu()]);this.closeLoadPanel('main');this.translate.saveFileTranslate();window.localStorage.setItem('appInfo',JSON.stringify(this.appInfo));window.localStorage.setItem('app.translate',JSON.stringify(this.translate.translate));window.localStorage.setItem('appInfo.date',new Date());}
await loadSettings;}
