...
async shouldLoadStructure(){return true;let lastDate=await this.processData('site/getstructurelastdate','POST',null);if(true){const appInfo=JSON.parse(window.localStorage.getItem('appInfo'));const translate=JSON.parse(window.localStorage.getItem('app.translate'));if(!appInfo||!translate)return true;this.appInfo=appInfo;this.translate.translate=translate;}else
...
disableButtons(isEdit,type,index){$.each(this.items,function(_,item){if(item.name=='buttonEdit'||item.name=='buttonDelete'||item.name=='buttonRefresh'||item.name=='buttonSearch'||(type=='edit'&&item.name=='buttonGridSort')||(type=='sort'&&item.name=='buttonAdd')){item.visible=!isEdit;if(index)
item.sortIndex=index;}});this.object.repaint();}
...
console.log(this.type);this.isEdit=false;this.mylsObject.mode='sel';this.showEditButtons('row');if(this.type=='cards'){this.exitBtn.option('visible',false);this.GridSortBtn.option('icon',this.getSortedIcon());$('#'+this.mylsObject.idn+' .dx-scrollview-content.card-view').sortable('destroy');this.mylsObject.refresh();}}
getSortedIcon(){return'img/sort.svg';}
getExitBtnOptions(){const self=this;return{icon:"img/revert.svg",elementAttr:{toolbarrole:"always",buttonrole:"exit",},visible:(this.positionColumn&&this.mylsObject.tableInfo.e==1)?false:true,onInitialized:e=>{self.exitBtn=e.component;},onClick:function(e){e.event.stopPropagation();self.exitFromEditing(e);}};}
...
getFilterBtnOptions(){const self=this;return{elementAttr:{toolbarrole:"always",buttonrole:"search",},icon:"img/filter.svg",onClick:function(e){self.mylsObject.filter.init(e);}};}
...
getGridSortBtnOptions(disable=true){const self=this;return{elementAttr:{toolbarrole:"focused",buttonrole:"gridsort",class:'',},icon:this.getSortedIcon(),disabled:disable,onInitialized:e=>{self.GridSortBtn=e.component;},onClick:function(e){let cls=e.component.option('icon');if(cls.indexOf('sort')!=-1){self.startOrdered(e);self.disableButtons(true,'sort');self.exitBtn.option('visible',true);}else{self.disableButtons(false,'sort');self.saveOrdered(e);self.exitBtn.option('visible',false);}}};}
startOrdered(e){const self=this;e.component.option('icon','img/save.svg');if(self.type=='grid'){self.mylsObject.object.clearSorting();self.mylsObject.object.columnOption(this.positionColumn.dataField,"sortOrder",'asc');}
