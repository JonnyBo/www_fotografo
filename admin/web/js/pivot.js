class Pivot extends MylsObject {
    ...
    fillDataSources() {
        if (!this.template) {
            let rows = this.columns.getColumnsByColumnType('row');
            let columns = this.columns.getColumnsByColumnType('column');
            let datas = this.columns.getColumnsByColumnType('data');
            this.template = {};
            this.template = {
                report: [{
                    attributes: {caption: 'XXX'},
                    row: {field: this.getDataField(rows)},
                    column: {field: this.getDataField(columns)},
                    data: {field: this.getDataField(datas)}
                }]
            };
        }
