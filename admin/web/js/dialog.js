class Dialog {
	...
	getOptions(title, message, items) {
		return {
			width: "auto",
			height: "auto",
			maxWidth: app.appInfo.device.deviceType == 'phone'? "90%":"50%",
			maxHeight: app.appInfo.device.deviceType == 'phone'? "90%":"50%",
			...
			title: title,
			visible: false,
			dragEnabled: app.appInfo.device.deviceType == 'phone'? false:true,
