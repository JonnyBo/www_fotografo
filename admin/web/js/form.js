class Form extends MylsEditableObject {
	...
	constructor(table, ext_id, view, mode, tHistory, viewMode, params) {
		...
		this.type = 'form';
		this.popup = null;
		this.customFormObjects = ['tagbox', 'treeview', 'image', 'colorbox', 'buttongroup', 'html', 'file', 'code'];
	...
	async getData(ext_id) {
		const self = this;
		return new Promise(async (resolve, reject) => {
			try {
				if (this.mode != 'setup') {
					this.tData = app.processData('form/tabledata', 'post', this.prepareTableData(ext_id));
				} else this.tData = {'0': []};
	...
	async afterLoad() {
		...
		$("#" + this.idn).removeClass("invisible");
	...
	async createForm() {
		...
		if (this.template[0].formtype == 'tabs') {
			...
			this.tabPanel = $("#" + this.idn).dxTabPanel({
				...
				onSelectionChanged:() => console.log(1)*/
			}).dxTabPanel('instance');
		...
		$('#' + this.idn + ' .dx-box-flex[style*="flex-direction: row"] > .dx-box-item').addClass(' myls-flex-grow-1');
	}
	...
	async processTab(form, tabId, tabcontent, tabIndex) {
		if (tabcontent == 'object') {
			...
			const selParams = this.getSelParamsForObject(column.tableId);
			$("#" + tabId).append(app.getObjectContainer(idx));
			let obj = app.getObject(column.tableId, this.tableData[column.extField], "popup", form.items[0].objectType, 'sel', app.addHistory(column.extTargetField ? column.extTargetField : column.extField, this.tableData[column.extField], this.idn, this.tHistory, this.initialMode), 'compact', selParams);
			this.initExternalObject(obj, column, tabIndex);
			this.columns.setDataDependenciesFromObject(column, obj.selParams);

			column.editor = obj;
			column.customEditor = true;
			column.customSave = true;
		}
	...
	async initExternalObject(obj, column, tabIndex) {
		const self = this;
		obj.changed = () => {
	...
	async processFormItemsTemplates(form, tabId, formObj, tabIndex) {
		...
		$.each(form, function (key, item) {
			...
			if (item.hasOwnProperty("objectType")) {
				...
				if (item.objectType == 'image' || item.objectType == 'file') {
					item.template = function (itemData, itemElement) {
						self.createFileTemplate(itemData, itemElement, tabId, item, item.objectType);
					};
				}
				if (item.objectType == 'treeview') {
					item.template = function (itemData, itemElement) {
						...
						if (column !== undefined) {
							column.element = $(itemElement).closest('.dx-item');
				...
				if (item.objectType == 'button') {
					item.template = function (itemData, itemElement) {
						...
						if (column)
							column.element = $(itemElement).closest('.dx-item');
						self.initButton(itemElement, column, item);
					};
				}
				...
				if (item.objectType == 'code') {
					item.template = function (itemData, itemElement) {
						const idnf = tabId + '_' + itemData.dataField + '_code';
						const column = self.columns.columns[itemData.dataField];
						column.element = $(itemElement).closest('.dx-item');
						itemElement.append('<div id="' + idnf + '" class="myls-code"></div>');
						self.initCodeEditor(idnf, column, item);
					};
				}
			}
		...
		$.each(form, function (key, it) {
			if (form[key].hasOwnProperty("items") || form[key].hasOwnProperty("tabs")) {
				...
				if (column) {
					column.visible = true;
					column.form = formObj;
					const validationRules = self.dependencies.getValidationRules(column);
					if (validationRules) {
						form[key].validationRules = validationRules;
						column.validationRules = JSON.stringify(validationRules);
					} else
				...
				if (form[key].hasOwnProperty("editorType")) {
					...
					if (form[key].editorType == 'dxNumberBox') {
						...
						form[key].editorOptions.onInitialized = function (e) {
							self.initializeEditor(e, column, 'myls-form-number');
							let buttons = [];
					...
					if (form[key].editorType == 'dxTextArea') {
						form[key].editorOptions.onInitialized = function (e) {
							self.initializeEditor(e, column);
						};
						form[key].editorOptions.onDisposing = function (e) {
					...
					if (form[key].editorType == 'dxCheckBox') {
						form[key].editorOptions.onInitialized = function (e) {
							self.initializeEditor(e, column);
						};
						form[key].editorOptions.onDisposing = function (e) {
					...
					if (form[key].editorType == 'dxDateBox') {
						form[key].editorOptions.onInitialized = function (e) {
							let addClass = 'myls-form-date';
							if (form[key].dataType == 'datetime')
								addClass = 'myls-form-datetime';
							if (form[key].dataType == 'time')
								addClass = 'myls-form-time';
							self.initializeEditor(e, column, addClass);
						};
						form[key].editorOptions.onDisposing = function (e) {
							self.disposeEditor(column);
						};
					...
					if (form[key].editorType == 'dxDateBox' && form[key].dataType == 'date') {
						...
						form[key].editorOptions.type = 'date';
						//form[key].editorOptions.showDropDownButton = false;
					...
					if (form[key].editorType == 'dxDateBox' && form[key].dataType == 'datetime') {
						...
						form[key].editorOptions.pickerType = "rollers";
						form[key].editorOptions.calendarOptions.showTodayButton = false;*/
					}
	...
	async createObjectTemplate(itemData, item, itemElement, tabIndex) {
		...
		if (column !== undefined) {
			...
			this.loadPromise.then(() => {
				...
				//if (!column.editor) {
				const selParams = self.getSelParamsForObject(tableId);
				...
				self.initExternalObject(obj, column, tabIndex).then(()=> {
					self.columns.setDataDependenciesFromObject(column, obj.selParams);
				});
	...
	getSelParamsForObject(tableId) {
		const selParams = {};
		for (let param of app.appInfo.tables[tableId].selParams) {
			if (this.tableData.hasOwnProperty(param)) {
				selParams[param] = this.tableData[param];
			}
		}
		return selParams;
	}
	...
	createFileTemplate(itemData, itemElement, tabId, item, fileType) {
		...
		const column = self.columns.columns[itemData.dataField];
		column.element = $(itemElement).closest('.dx-item');
		column.elementIdn = idn;
		const params = this.getImageParams(column);
		let url = "documents/fileupload?field=" + idn;
		if (params)
			url += "&params="+params;
		this.loadPromise.then(() => {
			...
			column.editor = $("#" + idn + 'uploader').dxFileUploader({
				multiple: false,
			...
			$("#" + tabId + '_' + item.dataField + '_fileuploader .dx-fileuploader-button').addClass("order-1 mt-2 d-block dx-button-default dx-button-mode-outlined");
			$("#" + tabId + '_' + item.dataField + '_fileuploader .dx-fileuploader-button').removeClass("dx-button-normal dx-button-mode-contained");
		...
		function fileUploaded(e) {
			...
			if (filename !== '') {
				...
				if (column.extField) {
					...
					if (app.hasValue(self.columns.columns[column.extField]))
	...
	getFileTemplate(column) {
		if (!this.tableData) return null;
		const filename = this.tableData[column.dataField];
		const idn = column.elementIdn;
		if (filename) {
			if (column.objectType === 'image') {
				return `<img class="myls-field-image"  src="files/${filename}" /><div id="${idn}-buttonDeleteFile"></div>`;
			}
			if (column.objectType === 'file') {
	...
	refreshFileTemplate(column) {
		...
		$("#" + idn + 'uploader .dx-fileuploader-input-container').append(this.getFileTemplate(column));
		this.createFileDeleteButton(column);
	}

	createFileDeleteButton(column) {
		//const column = self.columns.columns[itemData.dataField];
		const self = this;
		if (this.tableData && this.tableData[column.dataField])
			$('#' + column.elementIdn + '-buttonDeleteFile').dxButton({
				icon: "clear",
				onClick: function () {
					self.tableData[column.dataField] = null;
					if (column.extField) {
						self.tableData[column.extField] = null;
						self.setFieldValue(null, self.columns.columns[column.extField], null);
					}
					self.refreshFileTemplate(column);
				}
			});
	...
	async initTreeView(item, column, form) {
		...
		const dropdown = $("#" + item).dxDropDownBox({
			valueExpr: "id",
			displayExpr: "item",
			dataSource: data,
	...
	initLookupValues(column) {
		...
		return app.processData('form/getlookupvalues', 'post', {
			'id': column.id,
			'params': JSON.stringify(params),
			'isTabler': this.tableInfo.isTabler
		});
	}

	initializeEditor(e, column, addClass) {
		column.editor = e.component;
		column.element = $(e.element).closest('.dx-item');
		if (addClass && column.element) {
			$(column.element).addClass(addClass);
		}
		if (DEBUG)
	...
	initTagBox(item, column, formItem, form) {
		...
		column.editor = $("#" + item).dxTagBox({
			...
			onFocusOut: (e) => {
				if (column.canAdd) {
					...
					if (text) {
						const isFind = self.findInDataSource(text, e.component.option('items'));
						if (isFind == -1)
			...
			onInitialized: function (e) {
				self.initializeEditor(e, column);
				e.component.option('buttons', self.getLookupButtons(e, column));
			},
			onDisposing: function (e) {
				self.disposeEditor(column);
			},
			dropDownButtonTemplate: (data, element) => {
				self.getDropDownButtonTemplate(data, element, column);
			},
			onContentReady: (e) => {
				self.initAddButton(e, column);
			},
			onCustomItemCreating: (e) => {
				self.getOnCustomItemCreating(e, column);
			}

			/*item.editorOptions.onItemClick = (e) => {
				self.initAddButton(e, column, e.itemData.item);
			};
			;*/
		...
		if (formItem.grouped) {
			...
			column.editor.getDataSource().paginate(false);
		}
	...
	updateTagBox() {
		...
		$.each(this.columns.columns, function (index, item) {
			if ((item.dataType == 'tagbox' || item.dataType == 'treeview' || item.dataType == 'buttongroup') && item.changed) {
				...
				params.isTabler = self.tableInfo.isTabler;
	...
	async addNewLookupItem(e, column, searchText) {
		const self = this;
		if (searchText && column.canAdd && !column.adding) {
			column.adding = true;
	...
	inlineAddNewValue(column, values, e) {
		...
		$.ajax({
			...
			data: ({'id': column.id, 'params': values, 'isTabler': this.tableInfo.isTabler}),
	...
	async initButtonGroup(item, column) {
		...
		column.customEditor = true;
		column.customSave = true;
		column.element = $(item).closest('.dx-item');
	...
	async initButton(item, column, form) {
		...
		$(item).dxButton({
			...
			onClick: function () {
				...
				if (form.sendUserInvitation) {
					self.buttonRegUser(form);
				}
			}
		});
	}
	...
	async buttonRegUser(form) {
		try {
			await this.save(this.ext_id);
			const result = await app.processData('site/regbyuserurl', 'get', {'user_id': this.ext_id});
			if (result && result.success) {
				await app.dialog.showInfo(this.idn, app.translate.saveString('Приглашение отправлено пользователю на почту'));
			}
		} catch (error) {
			this.processResult(error);
			this.disableButtons(false);
	...
	async openFormDependencies() {
		...
		await Promise.all(pr);*/
		// Проставляем класс, если поле - обязательное
		...
		$.each(self.columns.columns, async (_, column) => {
			if (column.form) {
				self.dependencies.init(column.form, self.tableData);
				//await self.dependencies.setFieldValidation(column);
	...
	initLookup(item, column, form) {
		...
		if (item.grouped) {
			...
			item.editorOptions.dataSource.paginate = false;
		}

		setLookupItemTemplate();
		item.editorOptions.dropDownButtonTemplate = (data, element) => {
			self.getDropDownButtonTemplate(data, element, column);
		};
		...
		if (column.canAdd) {
			...
			item.editorOptions.onCustomItemCreating = (e) => {
				self.getOnCustomItemCreating(e, column);
			};
		}
		...
		item.editorOptions.onOptionChanged = function (e) {
			if (e.name === "text" || e.name === 'selectedItem') {
				self.initMoreButton(e, column);
				const customAddBtn = e.component.getButton("Add");
				if (customAddBtn) customAddBtn.mylsCurrValue = e.value;
			}
		};
		item.editorOptions.onContentReady = function (e) {
			self.initAddButton(e, column);
		};
	...
	getOnCustomItemCreating(e, column) {
		if (!e.customItem) {
			e.customItem = e.text;
		}
		if (e.text) {
			const isFind = this.findInDataSource(e.text, column.items);
			if (isFind == -1)
				this.addNewLookupItem(e, column, e.text);
		}
	}

	getLookupButtons(e, column) {
		const btns = ['clear'];
		if (column.canAdd) btns.push(this.getAddButtonOptions(e, column));
		if (column.extFormId) btns.push(this.getMoreButtonOptions(e, column));
		btns.push('dropDown');
		return btns;
	}

	getDropDownButtonTemplate(data, element, column) {
	...
	initMoreButton(e, column) {
		...
		if (customBtn) {
			...
			if (column.extFormId) {
				...
				customBtn.mylsCurrValue = currValue;
				if (currValue) {
	...
	initAddButton(e, column, currText = '') {

		const customAddBtn = e.component.getButton("Add");
		const customMoreBtn = e.component.getButton("More");
		if (customAddBtn || customMoreBtn) {
			if (customAddBtn) {
				customAddBtn.option("visible", false);
			}
			if (customMoreBtn)
				customMoreBtn.option("visible", false);

			if (!currText) currText = e.component.option("text");
			if (currText && typeof currText == 'string')
				currText = currText.trim();
			else
				currText = undefined;
			//let isFind = (e.value) ? e.value : -1;
			let isFind = -1;
			if (currText)
				isFind = this.findInDataSource(currText, column.items);
			if (customAddBtn) {
				customAddBtn.option("visible", currText && (isFind === -1) && column.canAdd);
				customAddBtn.mylsCurrValue = currText;
			}
			if (customMoreBtn)
	...
	async initCodeEditor(idnf, column, form) {
		//console.log(column.columnType);
		column.customEditor = true;
		column.customSave = false;
		if (column.editor) column.editor.destroy();
		if (column.columnType && $.isArray(column.columnType)) {
			let test = this.columns.getColumnTypeItem('test', column);
			let lang = this.columns.getColumnTypeItem('lang', column);
			let objTest = this.columns.getColumnTypeParameters(test);
			let objLang = this.columns.getColumnTypeParameters(lang);

			//console.log(objTest);
			//console.log(objLang);
			$('#' + idnf).css('min-height', '300px');
			column.editor = await ace.edit(idnf);
			column.editor.setTheme("ace/theme/tomorrow");
			//column.editor.session.setMode("ace/mode/" + objLang[0]);
			column.editor.getSession().setMode({path:"ace/mode/" + objLang[0], inline:true});

			this.setCustomEditorsValue(column, this.tableData[column.dataField]);
			const options = this.getTestButtonOptions(idnf, column.editor, objTest.url);
			this.addButtonTest(idnf, options);
		}

	}

	addButtonTest(idnf, options) {
		console.log($('#'+idnf+'-mylsTestButton').length);
		if ($('#'+idnf+'-mylsTestButton').length == 0) {
			$('#' + idnf).after('<div id="'+idnf+'-mylsTestButton"></div>');
			$('#'+idnf+'-mylsTestButton').dxButton(options);
		}
	}

	createTestInfo(idnf) {
		$('#'+idnf+'-mylsTestButton').after('<div id="' + idnf + '-info"></div>');
	}

	getTestButtonOptions(idnf, editor, url) {
		const self = this;
		return {
			stylingMode:"outlined",
			text: app.translate.saveString("Тестировать"),
			type: "default",
			onClick: async function() {
				const result = await app.processData(url, 'POST', {code: editor.getValue()});
				if ($('#'+idnf+'-info').length == 0)
					self.createTestInfo(idnf);
				if (result.success)
					$('#'+idnf+'-info').addClass('success').text(result.success);
				if (result.error)
	...
	getAddButtonOptions(e, column, currValue) {
		...
		return {
			...
			options: {
				...
				visible: false,
				onClick: function (el) {
					self.addNewLookupItem(e, column, el.component.mylsCurrValue);
					el.element.remove();
				}
			}
		};
	...
	getMoreButtonOptions(e, column, currValue) {
		...
		return {
			...
			options: {
				...
				visible: false,
				onClick: async function (el) {
					await self.editInExternalForm(column, el.component.mylsCurrValue, 'upd');
				}
			}
		};
	}
	...
	async editInExternalForm(column, extId, mode, text) {
		...
		//this.setButtonsVisible(column.editor, column);
		if (mode === 'ins' && extId) {
	...
	setCustomEditorsValues() {
		...
		$.each(this.columns.columns, async (_, item) => {
			...
			if (item.objectType == 'file' || item.objectType == 'image') {
				this.refreshFileTemplate(item);
			}
		});
	}
	...
	setCustomEditorsValue(item, values) {
		...
		if (item.editor)
			switch (item.dataType) {
				...
				case 'code'    :
					if (values)
						item.editor.setValue(values);
					item.editor.session.on('change', function(delta) {
						self.tableData[item.dataField] = item.editor.getValue();
					});
					break;
			}
		item.changed = false;
		...
		$.each(this.columns.columns, function (index, item) {
			if (item.editor)
				switch (item.dataType) {
					...
					case 'code':
						self.objectValues[item.dataField] = item.editor.getValue();
						break;
	...
	async cancelBtnClick() {
		...
		try {
			...
			if (this.rawMode === 'updins' || this.rawMode === 'ins') {
	...
	async reloadData(mode) {
		...
		//await getData(ext_id)
		this.tableData = await loadData;
	...
	async validate() {
		...
		$.each(this.forms, function (index, value) {
			promises.push(self.validateForm(value));
		});

		let result = await Promise.all(promises);
		$.each(this.forms, function (index, value) {
			isValid = result[index];
	...
	validateForm(form) {
		...
		return new Promise(async (resolve, reject) => {
			try {
				...
				let isValid = res ? true : res;
				if (res.status == 'pending') {
					const result = await res.complete;
	...
	setFieldVisible(form, column, isVisible) {
		//console.log(column.dataField, column.element, isVisible);
		if (!column.form || !column.element) return;
		...
		if (isVisible)
			$(column.element).removeClass('d-none');
		else
			$(column.element).addClass('d-none');
		column.visible = isVisible;
	}

	setFieldCaption(form, column, caption) {
		if (!column.form || !column.element) return;
		$(column.element).find('.dx-field-item-label-text').text(caption.text);
		/*let path = this.columns.getFieldPath(column);
	...
	setRequiredClass() {
		$.each(this.columns.columns, async (_, column) => {
			if (column.element) {
				$(column.element).removeClass('myls-required');
				if (column.setRequiredClass) {
					$(column.element).addClass('myls-required');
				}
			}
		});
	}
