class Filter {
    ...
    async processDefaultFilter() {
        this.filter = $('#' + this.mylsObject.idn + '_mylsFilter').dxFilterBuilder('instance').getFilterExpression();
        //console.log(this.filter);
        if (this.filter.length == 0) {
            this.filter = null;
        }
        ...
        this.mylsObject.dataSource.mylsFilter = this.filter;
        this.mylsObject.refresh();
        //this.mylsObject.dataSource.filter(this.filter);
        this.popup.hide();
    }
    ...
    processContains(elem) {
        if (elem.option('value')) {
            let out = [];
            out.push([elem.column.dataField.toLowerCase(), 'containing', elem.option('value')]);
            if (out.length > 0) {
                if (this.filter.length > 0) {
                    this.filter.push('and');
                }
    ...
    createDefaultFilter() {
        //let filter = [];
        const $filter = '<div id="' + this.mylsObject.idn + '_mylsFilter"></div>';
        $('#' + this.mylsObject.idn + '_popupContainer .myls-filters').append($filter);
        $('#' + this.mylsObject.idn + '_mylsFilter').dxFilterBuilder({
            fields: this.cols,
            value: this.filter
        });
    }
