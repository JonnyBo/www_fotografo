...
$dataSQL = $this->selectAll("select s.unit_price,
       s.quantity,
       iif(s.sys_rate_type_id = 2, s.rate_discount, s.amount_discount) as amount_discount,
       iif(s.sys_rate_type_id = 2, '%', '&euro;') discount_symbol,
       p.amount as amount_with_tax,
       s.comment,
       coalesce(get_price_templated_text(p1.template_for_account, d.discipline, l.level_name, p1.price_name), p1.price_name) as price_name,
       ...
       p1.sys_price_category_id,
       p.description purpose_payment,
       s.sys_rate_type_id
from payment_per_service p
   ...
   left outer join prices p1 on (p1.price_id = s.price_id)
where p.payment_id = :id", $params, $this->ddb);
...
foreach ($prices as $price) {
    ...
    if ($price['amount_discount'] != 0)
        if ($price['sys_rate_type_id'] != 2)
            $price['amount_discount'] = number_format($price['amount_discount'], 2, ',', '.').'&euro;';
        else
            $price['amount_discount'] = number_format($price['amount_discount'], 0, ',', '.').'%';

    $discount = $price['amount_discount'] != 0 ? $price['amount_discount'] : '';
