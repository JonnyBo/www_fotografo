...
$resSQL = $this->selectAll("select a.account_num,
       ...
       t.territory as city,
       iif(cl.sys_type_person_id = 2, cl.inn, cd.document) as inn,
       amount_services_account(a.account_id) sum_account,
       a.payer_id,
       cl.sys_type_person_id,
       lpad(extract(day from a.pay_to), 2, '0') || '/' || lpad(extract(month from a.pay_to), 2, '0') || '/' || lpad(extract(year from a.pay_to), 4, '0') as pay_to
from accounts a
...
if ($data['pay_to'])
       echo' <p style="font-size:20px;">Fecha de vencimiento: '. $data['pay_to']. '</p>';

        echo '<p style="text-align: left;margin-top: 100px;font-size: 20px;" class="cyan">Gracias por confiar en nosotros</p>
    </main>
</body>
