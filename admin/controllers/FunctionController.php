...
trait FunctionController
{
    ...
    protected function getMainSQLs()
    {
        ...
        if ($firstData === false) {
            ...
            $firstData = $this->selectAll($firstSql, [], $this->db)['success'];
    ...
    protected function setTransacionUser($db) {
        ...
        //$this->selectAll($sql, $params, $db)['success'];
    }
    ...
    protected function mustLogout()
    {
        //Yii::info($_REQUEST, 'dev_log');
        if (Yii::$app->user->isGuest) {
            Yii::$app->session->setFlash('error', 'Сессия пользователя окончена.');
