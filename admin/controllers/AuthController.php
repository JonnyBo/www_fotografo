...
class AuthController extends \yii\web\Controller
{
    ...
    public function actionRegbyuser()
    {
        $this->layout = 'main_login';
        $error = [];
        $token = Yii::$app->request->get('token', false);
        ...
        $user_id = intval(Yii::$app->getSecurity()->decryptByKey(urldecode($token), Yii::$app->components['request']['cookieValidationKey']));
        if ($user = User::findIdentity($user_id)) {
            if ($user->status > 1) {
                //пользователь уже зареген
                Yii::$app->getSession()->setFlash('error', 'Пользователь уже зарегистрирован!');
                return $this->redirect('login');
            }
        }
        ...
        if ($model->load(Yii::$app->request->post(), '')) {
            ...
            $model->login = User::getLogin($user_id);
            //$model->company_id = $company_id;

            if ($user = $model->update()) {
                //$user->login = User::getLogin($user->user_id);
                //print_r($user);
                //exit();
                $model->sentEmailConfirm($user);
