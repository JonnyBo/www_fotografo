...
class FrameController extends Controller
{
    ...
    protected function getAllTablesColumns()
    {
        ...
        if ($result === false) {
            ...
            for ($i = 0; $i < count($columns); $i++) {
                ...
                $colByAlias[$tableId][$field]['parentId'] = $columns[$i]['parent_column_id'];
                $colByAlias[$tableId][$field]['columnId'] = $columns[$i]['column_id'];
                ...
                if ($columns[$i]['footer_field'])
                    $colByAlias[$tableId][$field]['footerField'] = $columns[$i]['footer_field'];
                ...
                if (in_array($colByAlias[$tableId][$field]['dataType'], ['url', 'file', 'image'])) {
                ...
                if ($columns[$i]['is_tabler'] !== null)
                    $colByAlias[$tableId][$field]['isTabler'] = $columns[$i]['is_tabler'];

                $tCols[$columns[$i]['table_id']][] = $columns[$i];

                if ($columns[$i]['footer_function'] !== null && $columns[$i]['is_use_to_view']) {
                    $cur_sum = $sum[$tableId][] = ['column' => $colByAlias[$tableId][$field]['dataField'],
    ...
    protected function get_footer_column($dataField, $columns)
    {
        for ($i = 0; $i < count($columns); $i++) {
    ...
    protected function parseColumnTypes($column_type)
    {
        $res = [];
        $pattern = '/\[(.+?)\]/';
        $arrType = explode(';', $column_type); // Если в записи не присутствует ; то возвращается пустой массив,
        //поэтому неправильно обрабатываются строки типа filter[required], чтобы работало, приходится писать что-то типа filter[required];ccc
        if ($arrType) { // Тут непонятно, почему обрабатывается только массивы от 2 значений и больше. А что с массивом, где всего один элемент? он тоже должен обрабатываться
    ...
    */
    protected function convertTableToApp()
    {
        ...
        if (isset($this->tableInfo['copy_procedure'])) {
            $ti['copyProc'] = $this->function->getParamFromSql($this->tableInfo['copy_procedure'], true);
        }

        if (isset($this->tableInfo['documents_procedure'])) {
            $ti['docProc'] = $this->tableInfo['documents_procedure'];
        }
        if ($ti['tableType'] == 'documents') {
            $ti['docProc'] = '=loadfiles';
            $ti['tableType'] = 'grid';
        }
        ...
        if (isset($this->tableInfo['is_tabler']))
            $ti['isTabler'] = $this->tableInfo['is_tabler'];
        else
            $ti['isTabler'] = 0;
        return $ti;
    }
    ...
    */
    ...
    function actionTabledata(/*$id, $extId = null, $mode = null*/)
    {
        ...
        $isTabler = Yii::$app->request->post('isTabler', 0);
        ...
        $tdb = $isTabler == 0 ? $this->ddb : $this->db;

        $transaction = $tdb->beginTransaction();
        try {
            // Добавление данных
            ...
            if ($mode == 'ins') {
                ...
                $valuesList = $this->selectOne($insertDefaultSql, [], $tdb)['success'];
            ...
            $dataList = $this->selectAll($req['sql'], $req['params'], $tdb)['success'];
            if ($mode == 'ins')
                $transaction->rollback();
            else
                $transaction->commit();

            $this->putStatistic($id, null, (microtime(true) - $start) * 1000, null, $params);
    ...
    function actionDelete($table, $extId, $lang)
    {
        ...
        if ($sql != '') {
            ...
            if (strpos($sql, 'select') === false)
                return $this->executeSQL($sql, $params, $tdb);
            else {
                $resSQL = $this->selectOne($sql, $params, $tdb);
                if (isset($resSQL['success'])) {
                    $result = $resSQL;
                } else {
                    return json_encode(array(
    ...
    function actionGetlookup()
    {
        ...
        if ($params) {
            $resSQL = $this->selectAll($sql, $params, $tdb);
        } else {
            $resSQL = $this->selectAll($sql, array(), $tdb);
        }

        $this->putStatistic(null, $id, (microtime(true) - $start) * 1000, null, $params, 'lookup');
    ...
    public
    function actionGetlookupvalues()
    {
        $mustLogout = $this->mustLogout();
        if ($mustLogout)
            return $mustLogout;
        ...
        if ($params) {
            $resSQL = $this->selectAll($sql, $this->function->convertArrayToParams($params), $tdb);
        } else {
            $resSQL = $this->selectAll($sql, array(), $tdb);
        }

        $this->putStatistic(null, $id, (microtime(true) - $start) * 1000, $sql, $params, 'lookup_values');
        if (isset($resSQL['success'])) {
            $result = $resSQL['success'];
        } else {
            throw new HttpException(500, $resSQL['error']);
    ...
    function actionInsertlookup()
    {
        ...
        if ($id && $params) {
            ...
            if ($params) {
                $resSQL = $this->selectOne($sql, $this->function->convertArrayToParams($params), $tdb);
            } else {
                $resSQL = $this->selectOne($sql, array(), $tdb);
            }
    ...
    function actionUpdatelookup()
    {
        ...
        if ($id && $params) {
            ...
            if ($params) {
                $result = $this->executeSQL($sql, $this->function->convertArrayToParams($params), $tdb);
            } else {
                $result = $this->executeSQL($sql, array(), $tdb);
            }
        } else $result = [];
        return json_encode($result);
    }
    ...
    function actionUpdate()
    {
        ...
        return $this->update($table, $ext_id, $data, $type, $isTabler);
    }

    protected function update($table, $ext_id, $data, $type, $isTabler = 0)
    {
        if (count($data) == 0) {
            return json_encode(array(
                'success' => true
        ...
        if ($type == 'upd') {
            ...
            if (count($this->tableInfo->updParams))
                return $this->executeSQL($updateSQL, $params, $tdb);
            else {
                $sql = '';
                foreach ($data as $key => $value) {
                    if ($sql != '') $sql .= ", \n";
        ...
        if ($type == 'ins') {
            ...
            if ($pos !== false) {
                ...
                return $this->executeSQL($insertSQL, $params, $tdb);
            }
        }
    }
    ...
    public function actionCheckdata()
    {
        ...
        if ($mustLogout)
            return $mustLogout;
        return $this->execProc('check_procedure', true);
    }
    ...
    public function actionUpdateproc()
    {
        ...
        if (strpos($updateSQL, 'select') !== -1)
            $msg = json_encode($this->selectOne($updateSQL, $params, $tdb));
        else
            $msg = $this->executeSQL($updateSQL, $params, $tdb);
        return $msg;
    }
    ...
    protected function execProc($procType, $use_select = false)
    {
        $start = microtime(true);
        $table = Yii::$app->request->post('table');
        $params = Yii::$app->request->post('params');
        ...
        if (!$use_select) {
            ...
            $msg = $this->executeSQL($sql, $params, $tdb);
        } else {
            $sql = 'select error_msg, error_type from ' . $this->tableInfo[$procType];
            $msg = $this->selectAll($sql, $params, $tdb);
        }
        $this->putStatistic($table, null, (microtime(true) - $start) * 1000, $sql, $params, $procType);
    ...
    public function actionGetalltemplates()
    {
        ...
        if ($result === false) {
            ...
            foreach ($tables as $idx => $table) {
                ...
                } elseif (in_array($table['table_type'], ['pivot', 'chart']) && $table['form_xml']) {
                    $result[$idx] = $this->parser->parsePivot($table['form_xml']);
                } elseif (in_array($table['table_type'], ['cards', 'draglist']) && $table['form_xml']) {
                    $result[$idx] = $this->parser->parseColumn($table['form_xml']);
                    //$result[$idx] = $this->parser->getXmlTemplate($table['form_xml']);
                } elseif (in_array($table['table_type'], ['dashboard', 'layout']) && $table['form_xml']) {
                    $result[$idx] = $this->parser->parseDashboard($table['form_xml']);
                } elseif ($table['form_xml']) {
                    $result[$idx] = $this->parser->parsePivot($table['form_xml']);
                }
            }
            $this->saveCache($key, $result);
    ...
    public function actionGetFilterStringData()
    {
        ...
        if ($req = $this->getSqlForFilterData($table, $field, $extId, $selParams)) {
            ...
            $dataList = $this->selectAll($sql, $req['params'], $this->ddb)['success'];
    ...
    public function actionAllsearchlookupexists()
    {
        $mustLogout = $this->mustLogout();
        if ($mustLogout)
            return $mustLogout;
        $this->getQuickActionsSQL();
        return ($this->mainSQLs['alllookup'] ? json_encode(['success' => 'true']) : null);
    }
    ...
    public function actionGetallsearchlookup()
    {
        ...
        if (!$this->mainSQLs['alllookup'])
            $this->getQuickActionsSQL();
        $sql = $this->mainSQLs['alllookup'];
    ...
    public function actionUpdateposition()
    {
        ...
        $ext_id = Yii::$app->request->post('ext_id');
        $data = Yii::$app->request->post('data');
        $dataField = Yii::$app->request->post('dataField');
        $data = json_decode($data);
        foreach ($data as $key => $item) {
            $res = $this->update($table, $item, [$dataField => $key], 'upd');
    ...
    public function actionProcessselect()
    {
        $mustLogout = $this->mustLogout();
        if ($mustLogout)
            return $mustLogout;
        ...
        $id = Yii::$app->request->post('id');
        $rows = json_decode(Yii::$app->request->post('rows'));
        $sql = 'select error_msg, error_type from ' . $func;
        foreach ($rows as $row) {
            $params = [
                ':id' => $id,
                ':ext_id' => $row
            ];
            $result[] = $this->selectOne($sql, $params, $this->ddb);
        }
        //}
        return json_encode(array(
            'success' => true,
    ...
    protected function putStatistic($table_id, $sql_id, $time, $sql, $params, $type = 'select')
    {
        $ssql = 'insert into statistic (user_id, table_id, sql_id, sql_text, params, running_time, stype)
                 values (:user_id, :table_id, :sql_id, :sql_text, :params, :running_time, :stype)';
        $sparams = [':user_id' => $this->userID, ':table_id' => $table_id, ':sql_id' => $sql_id, ':sql_text' => $sql, ':params' => json_encode($params), ':running_time' => $time, ':stype' => $type];
        $this->executeSQL($ssql, $sparams, $this->db);
    }

    protected function getAllSettings()
    {
        $uset_id = Yii::$app->user->id;
        if (!$uset_id)
            return false;
        $sql = 'select table_id, setting_value from table_settings where user_id = :user_id';
        $params = [':user_id' => $uset_id];
        $result = $this->selectAll($sql, $params, $this->db)['success'];
        return $result;
    }
    ...
    public function actionGetallsettings()
    {
        ...
        foreach ($settings as $s) {
            $result[$s['table_id']] = json_decode($s['setting_value']);
        }
        return json_encode($result);
    }
}