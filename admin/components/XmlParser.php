...
class XmlParser extends Component
{
    ...
    private function processFormInputTag($arr, &$template, $dir, $colCount = 1)
    {
        ...
        if (isset($arr['attributes']['grouped']) && $arr['attributes']['grouped'] == 'true') {
            $template['grouped'] = true;
        }
        // Временно отключаем все группы
        $template['grouped'] = false;
        ...
        if (isset($arr['attributes']['type'])) {
            switch (strtolower($arr['attributes']['type'])) {
                ...
                case 'html':
                    ...
                    break;
                case 'code':
                    $template['objectType'] = 'code';
                    break;
                case 'layout':
                    $template['objectType'] = 'layout';
                    break;
                default:
                    $template['objectType'] = strtolower($arr['attributes']['type']);
            }
        }
    }
    ...
    private function processFormButtonTag($arr, &$template)
    {
        ...
        if (isset($arr['attributes']['senduserinvitation'])) {
                $template['sendUserInvitation'] = true;
        }
