...
class LoginForm extends Model
{
    ...
    public function login()
    {
        //try {
            if ($this->validate()) {
                if ($user = $this->getUser()) {
                    if ($user->status === User::STATUS_WAIT || $user->status === User::STATUS_REGISTER) {
                        //Yii::$app->session->setFlash('error', 'Проверьте свой E-mail');
                        throw new \Exception('Проверьте свой E-mail');
                    }
                    if (!User::getActive($user->user_id)) {
                        throw new \Exception('Период активации пользователя окончен.');
